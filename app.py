...
# ---Function to Create Visualization from Excel ---
...
def create_visualization_from_excel(file_path):
    try:
        ...
        # Get column names for category and numeric value
        x_col = df.columns[0]
        y_col = df.columns[1]

        # Ensure the second column is numeric; force conversion if not
        ...
        # Create a unique identifier for each row to guarantee one bar per row
        df['unique_id'] = df.index.astype(str) + "_" + df[x_col].astype(str)

        # Extract lists for plotting
        unique_ids = df['unique_id'].tolist()   # unique x-axis values
        categories = df[x_col].tolist()           # original category labels
        values = df[y_col].tolist()               # numeric values

        # Create a bar chart using Graph Objects, which does not aggregate data by default
        fig = go.Figure(data=[go.Bar(
            x=unique_ids,   # Unique x-values to ensure one bar per row
            y=values,
            text=values,    # Use the numeric values for text labels on each bar
            marker_color='blue',
            customdata=categories,  # Store the original category labels for hover text
            hovertemplate=(
                f"{x_col}: %{ { 'customdata' } }<br>"
                f"{y_col}: %{ { 'y' } }<br>"
                "<extra></extra>"
            )
        )])

        # Update the layout to show the original category labels on the x-axis
        fig.update_layout(
        ...
        # Convert the figure to JSON for transmission
        ...
        print(f"Error in create_visualization_from_excel: {e}")
